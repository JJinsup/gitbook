...
```
Action: SEARCH_HEART
Response: 아직 하트 카드는 안 보여서 제자리에서 돌면서 더 찾아볼게요.
```
...
Search 모드가 활성화되면 로봇은 다음과 같이 동작합니다.
...
> **\[이미지]** **Search 모드 중 제자리 회전하는 로봇**

> **\[이미지]** **YOLO 화면에서 목표 카드가 새롭게 감지되는 순간**

이 과정에서 LLM은 추가로 개입하지 않습니다. 탐색 중단 여부는 전적으로 \*\*인지 이벤트(YOLO 감지)\*\*에 의해 결정됩니다.
