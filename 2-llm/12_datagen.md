---
description: >-
  이 문서는 PDF 문서 하나만 있으면, LLM 학습(Fine-tuning)에 필요한 고품질의 질문-답변(Q&A) 데이터셋을 자동으로 생성하고
...
LLM을 특정 도메인(법률, 의학, 사내 규정 등)에 맞게 미세 조정(Fine-tuning)하려면 양질의 Q\&A 데이터가 필수적입니다. 하지만 사람이 직접 수천 개의 질문과 답변을 만드는 것은 시간과 비용이 많이 듭니다.
...
```
pip install google-genai python-dotenv langchain langchain-community pymupdf tqdm
```
